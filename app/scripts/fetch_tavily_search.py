...
from app.services.tavily_search_service import asearch_google_batch, search_google
...
def _search_parameter_lines(payload: dict) -> list[str]:
    ...
    if not (gl or hl):
        return []
    return [f"Source: Tavily Web Search (gl={gl or '-'}, hl={hl or '-'})", ""]
...
def _write_result(output_dir: Path, query: str, payload: dict):
    file_stem = f"tavily-web-{_slug(query)}"
...
def main():
    parser = argparse.ArgumentParser(description="Fetch web search results via Tavily.")
    parser.add_argument(
