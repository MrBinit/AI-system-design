...
if [[ -n "${ENV_FILE}" && -f "${ROOT_DIR}/${ENV_FILE}" ]]; then
  ...
  echo "[local-up] No env file configured; using process env + AWS secrets only."
fi
...
"${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" build api gradio

echo "[local-up] Starting redis/api/worker/llm-worker/eval-worker/metrics-worker/gradio (gradio:${GRADIO_PORT})..."
"${COMPOSE_CMD[@]}" "${COMPOSE_ARGS[@]}" up -d --no-build redis api worker llm-worker eval-worker metrics-worker gradio
...
echo "[local-up] ENV:    ${ENV_FILE:-<none>} (optional)"
