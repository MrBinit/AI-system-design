...
if curl --noproxy "*" --max-time 10 -fsS "http://127.0.0.1:${API_PORT:-8000}/healthz" >/tmp/local-healthz.out 2>/tmp/local-healthz.err; then
  ...
  docker compose "${COMPOSE_ARGS[@]}" exec -T api python -c "import urllib.request; print(urllib.request.urlopen('http://127.0.0.1:8000/healthz', timeout=5).read().decode())"
fi

echo "[local-smoke] Checking Redis connectivity from API container..."
docker compose "${COMPOSE_ARGS[@]}" exec -T api python -c "from app.infra.redis_client import app_redis_client as c; print(c.ping())"
...
if curl --noproxy "*" --max-time 10 -fsS "http://127.0.0.1:${GRADIO_PORT}" >/dev/null 2>/tmp/local-gradio.err; then
  ...
  docker compose "${COMPOSE_ARGS[@]}" exec -T gradio python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:7860', timeout=5); print('gradio_ok=true')"
fi

if [[ "${RUN_CHAT}" -eq 1 ]]; then
  echo "[local-smoke] Running one authenticated /chat call..."
  TOKEN="$(docker compose "${COMPOSE_ARGS[@]}" run --rm --no-deps api python -m app.scripts.generate_jwt --user-id local-user --roles admin | tail -n 1)"
  if ! curl --noproxy "*" --max-time 30 -fsS "http://127.0.0.1:${API_PORT:-8000}/api/v1/chat" \
    ...
    docker compose "${COMPOSE_ARGS[@]}" exec -T -e TOKEN="${TOKEN}" api python - <<'PY'
import json
import os
